...
implements Util {
    public static int getItemSlot(Item item) {
        int itemSlot = -1;
        for (int i = 45; i > 0; --i) {
            if (!InventoryUtil.mc.player.inventory.getStackInSlot(i).getItem().equals((Object)item)) continue;
            itemSlot = i;
            break;
        }
        return itemSlot;
    ...
    public static void SilentSwitchToSlot(int slot) {
        if (InventoryUtil.mc.player.inventory.currentItem == slot || slot == -1) {
            return;
        }
    ...
    public static void switchToHotbarSlot(Class clazz) {
        int slot = InventoryUtil.findHotbarBlock(clazz);
        if (slot != -1) {
            InventoryUtil.switchToHotbarSlot(slot);
        }
    }
    ...
    public static boolean[] switchItem(boolean back, int lastHotbarSlot, boolean switchedItem, Class clazz) {
        ...
        if (!back && !switchedItem) {
            ...
            Mod.inventoryManager.recoverSilent(lastHotbarSlot);
        }
