...
extends Module {
    ...
    public Scaffold() {
        super("Scaffold", Module.Category.MOVEMENT, "i own deez towerz");
    }
    ...
    public void onUpdateWalkingPlayerPost(UpdateWalkingPlayerEvent event) {
        ...
        if (this.isDisabled() || Scaffold.fullNullCheck() || event.getStage() == 0) {
            return;
        }
    ...
    public void place(BlockPos posI, EnumFacing face) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (stack == null || stack.getItem() instanceof ItemAir || !(stack.getItem() instanceof ItemBlock) || !Block.getBlockFromItem((Item)stack.getItem()).getDefaultState().isFullBlock()) continue;
            newSlot = i;
            break;
        }
        if (newSlot == -1) {
        ...
        if (!Scaffold.mc.player.isSneaking() && BlockUtil.blackList.contains((Object)(block = Scaffold.mc.world.getBlockState(pos).getBlock()))) {
